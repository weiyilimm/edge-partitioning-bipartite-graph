...
Parameter
    ...
    e.g. {'a': {1}, 'b': {1, 2, 3}}
    The key indicates the nodes on the left 
    The values indicates the nodes on the right
    In the example, a and 1 are connected by an edge 
Return
    e_0, e_w, e1
'''
...
graphs = []
graphs = generate_sample_graph(10,50,60)
for x_to_y_graph in graphs:
    ...
    C = CostaImproved(x_to_y_graph)
    E1, Ew, E0 = C.edge_partitioning()
    
    if (E1 != e_1) or (Ew != e_w) or (E0 != e_0):
        print("Wrong")
        print("Matching: " + str((matching)))
        print("Graph: " + str(x_to_y_graph))
        print("e0" + str(e_0))
        print("ew: " + str(e_w))
        print("e1: " + str(e_1))
        print("E0: " + str(E0))
